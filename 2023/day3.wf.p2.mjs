// import { input as testInput } from './day3.wf.input.mjs';
  ...
  for (let row = 0; row < grid.length; row++) {
    ...
    for (let col = 0; col < grid[row].length; col++) {
      ...
      if (!isNaN(char) && char !== '.') {
        ...
        if (number !== '') {
          numberLocations.push({ number: parseInt(number, 10), positions: { row, startCol, endCol: col - 1 } });
          number = '';
        }
      }
    }
    if (number !== '') {
      numberLocations.push({ number: parseInt(number, 10), positions: { row, startCol, endCol: grid[row].length - 1 } });
    }
  }
...
function totalNumberValues(numbersWithAdjacentSymbols) {
  return numbersWithAdjacentSymbols.reduce((total, { number }) => total + number, 0);
}
