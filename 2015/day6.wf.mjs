...
// Initialize 1000x1000 2D array with 0 values
...
// Process each line
lines.forEach(line => {
    // Split the line by command options
    const [command, rest] = line.split(/(turn on|turn off|toggle)/).filter(Boolean);
    console.log("DB ... 🚀 ~ file: day6.wf.mjs:312 ~ command, rest ~~~", command, rest)
    // Replace 'through' with space and split by space to get coordinates
    const [start, end] = rest.replace('through', '*').trim().split('*');
    console.log("DB ... 🚀 ~ file: day6.wf.mjs:315 ~ start, end ~~~", start, end)

    const [startX, startY] = start.split(',').map(Number);
    console.log("DB ... 🚀 ~ file: day6.wf.mjs:318 ~ startX, startY ~~~", startX, startY)
    const [endX, endY] = end.split(',').map(Number);
    console.log("DB ... 🚀 ~ file: day6.wf.mjs:320 ~ endX, endY ~~~", endX, endY)
    console.log()
...
// Calculate the total value of all cells
const totalValue = grid.flat().reduce((sum, cell) => sum + cell, 0);
