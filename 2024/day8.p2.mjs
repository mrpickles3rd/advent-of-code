    ...
    for (const char in groups) {
        ...
        for (let i = 0; i < positions.length; i++) {
            for (let j = i + 1; j < positions.length; j++) {
                ...
                let k = 1;
                while (true) {
                    const antinodeX1 = a1.x - k * dx;
                    const antinodeY1 = a1.y - k * dy;
                    const antinodeX2 = a2.x + k * dx;
                    const antinodeY2 = a2.y + k * dy;

                    if (!antinodesWithinBounds(antinodeX1, antinodeY1, groupRows) && !antinodesWithinBounds(antinodeX2, antinodeY2, groupRows)) {
                        break;
                    }
...
// addHashes(testInputSimple);
// addHashes(testInputHard);
addHashes(input);
